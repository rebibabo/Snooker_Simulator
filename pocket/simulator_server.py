    ...
    def do_POST(self):
        ...
        if parsed_path.path == '/simulate':
            try:
                ...
                # 提取袋口参数（可选）
                pocket_tightness = float(params.get('pocket_tightness', 0.5))
                ...
                if initial_x!=0 or initial_y!=0 or initial_z!=0:
                    print(f"📍 Custom ball position: x={initial_x:.3f}, y={initial_y:.3f}, z={initial_z:.3f}")
                ...
                if enable_collision:
                    ...
                    # 创建红球（目标球）- 位置在白球前方（x方向）
                    red_ball = make_stationary_ball('red', np.array([0.3, 0.0]))
                    balls = [cue_ball, red_ball]
                    result_obj = integrate_multi(balls, tightness=pocket_tightness)
                    # 转换多球结果格式
                ...
                print(f"📦 Pocket tightness: {pocket_tightness:.2f}")
                ...
                response = {
                    ...
                    'pocket_tightness': float(pocket_tightness),
