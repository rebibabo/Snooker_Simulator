...
def make_corner_pocket(name: str, corner_x: float, corner_y: float,
    ...
    参考斯诺克台面几何：
    - 上库 (y = TABLE_Y_MAX) 和下库 (y = TABLE_Y_MIN) 是水平边
    - 左库 (x = TABLE_X_MIN) 和右库 (x = TABLE_X_MAX) 是竖直边
    - 四个角落是库边的交点
    """
...
def make_all_pockets(tightness: float = 0.5) -> List[Pocket]:
    """
    生成标准斯诺克台面的 6 个袋口（4 角 + 2 中）。
    
    参数：
      tightness: 0.0 (宽松) 到 1.0 (紧凑)
    
    返回：
      [tr, tl, br, bl, rc, lc] - 按照对称顺序
    """
    return [
        # 4 个角袋（按象限顺序）
        ...
        # 2 个中袋
        make_center_pocket('rc', +1, tightness),  # 右中袋
        make_center_pocket('lc', -1, tightness),  # 左中袋
    ]


# ── 落袋检测 ──────────────────────────────────────────────────────
def check_potted(pos_2d: np.ndarray, pockets: List[Pocket]) -> Optional[Pocket]:
    """
    检测球是否已进入任何袋口的吞球区。
    
    参数：
      pos_2d: 球的 2D 位置 [x, y]（不包括 z）
      pockets: 所有袋口列表
    
    返回：
      进球的 Pocket 对象，如果没有落袋则返回 None
    """
...
def check_jaw_collision(pos_2d: np.ndarray,
                       pockets: List[Pocket]) -> Optional[Tuple]:
    """
    检测球是否碰到袋颚弧。
    
    返回：
      (jaw_center, normal_hat, pocket, side_str) 如果有碰撞
      None 如果无碰撞
      
    其中：
      - jaw_center: 袋颚圆心
      - normal_hat: 单位法向量（从圆心指向球）
      - pocket: 袋口对象
      - side_str: 'left' 或 'right'
    """
    COLLISION_DIST_TOLERANCE = 0.005  # 5mm
    
    for pocket in pockets:
        for side, jaw_center in [
            ('left', pocket.jaw_left_center),
            ('right', pocket.jaw_right_center)
        ]:
            r_vec = pos_2d - jaw_center
            dist = np.linalg.norm(r_vec)
            
            # 检测碰撞：球表面距离 < 容限
            if pocket.jaw_radius < dist < pocket.jaw_radius + COLLISION_DIST_TOLERANCE:
                n_hat = r_vec / dist
                return (jaw_center, n_hat, pocket, side)
...
if __name__ == '__main__':
    # 简单测试
    print("Testing pocket.py...")
    pockets = make_all_pockets(tightness=0.5)
    print(f"✅ Created {len(pockets)} pockets")
    
    for p in pockets:
        print(f"  {p.name:3s}: type={p.pocket_type:6s}, "
              f"center={p.center}, capture_r={p.capture_radius:.4f}")
    
    # 测试检测
    tr = pockets[0]
    test_pos = tr.center.copy()
    result = check_potted(test_pos, pockets)
    print(f"\n✅ Potted test: {result.name if result else 'None'}")
"""
斯诺克袋口模块
================
6 个袋口（4 角袋 + 2 中袋）的几何定义、落袋检测、袋口弧形碰撞。
