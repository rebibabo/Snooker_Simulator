...
def step_single_ball(ball: BallEntity, dt: float,
    ...
    if hit_cushion:
        # 估计相对速度：碰撞前的速度就是相对速度
        rel_velocity = float(np.linalg.norm(v_before_cushion[:2]))  # 水平速度分量
        
        events.append({
            't': t,
            'type': 'cushion_hit',
            'ball': ball.ball_id,
            'pos': pos[:2].copy().tolist(),
        ...
        for j in range(i+1, n):
            ...
            if not np.allclose(v_a_new, v_a_before):
                ...
                events.append({
                    ...
                    'pos': ((ball_a.pos[:2] + ball_b.pos[:2]) / 2).tolist(),
                    'rel_velocity': float(rel_velocity),
                })
...
def integrate_multi(
    ...
    while t < max_time:
        ...
        # ① 每个球独立单步演化
        for ball in balls:
            step_single_ball(ball, dt, events, t)
